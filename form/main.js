...
//Variable til filtering
const settings = {
  filter: "all",
};
//Array for all beers in basket
let basketData = [];
...
function registerValidation() {
  ...
  document.querySelector("#cardNumber").addEventListener("change", validateCardnumber);

  document.querySelector("#month_year").addEventListener("change", validateMonth);
...
function checkValidation() {
  ...
  const validated = Object.values(validation).filter((val) => val === false).length === 0;
