...
function registerButtons() {
  ...
  document
    .querySelector("#dash-nav-web .logout-button")
  ...
  document
    .querySelector("#dash-nav-mobil .logout-button")
    .addEventListener("click", () => {
      localStorage.removeItem("login");
      window.location.href = `index.html?state=${realTime.stateParam}`;
    });
...
function showBeer(data) {
  ...
  data.taps.forEach((beer) => {
    ...
    clone.querySelector("img").src = new URL(
      `./assets/${beer.beer}.svg`,
      import.meta.url
    ).href;
    ...
    custumer.forEach((custumer) => {
      ...
      if (custumer.id === custNumber) {
        ...
        if (!beerSold.custumersServed.includes(custumer.id)) {
          ...
          beerSold[thisBartender].income =
            beerSold[thisBartender].sold * beerPrice;
        }
      }
    });
  });
